  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/auth/reset-password/validate?token=${encodeURIComponent(token)}`, {
      ...
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(body?.error || "Could not validate reset link");
        setTokenValid(Boolean(body?.valid));
      })
      .catch((err) => {
        if (err?.name !== "AbortError") {
          setTokenValid(false);
          setValidationError(err instanceof Error ? err.message : "Could not verify reset link");
        }
      })
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#efefef] px-4">
      <div className="w-full max-w-[360px]">
        <Card className="border border-[#d9d9d9] bg-white shadow-sm">
          ...
          <CardContent>
            {checkingToken ? (
              <p className="py-6 text-center text-sm text-muted-foreground">Checking reset link...</p>
            ) : validationError ? (
              <div className="py-4 text-center">
                <p className="text-sm font-medium text-destructive">Could not verify this reset link.</p>
                <p className="mt-2 text-sm text-muted-foreground">
                  Confirm the backend is running, then refresh this page.
                </p>
              </div>
            ) : tokenValid ? (
              ...
              <div className="py-4 text-center">
                <p className="text-sm font-medium text-destructive">This reset link is invalid, expired, or already used.</p>
                <p className="mt-2 text-sm text-muted-foreground">Use Forgot Password to request a new secure link.</p>
              </div>
            )}
