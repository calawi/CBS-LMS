...
import { buildCbsEmailTemplate, emailButton, emailParagraph, escapeEmailHtml } from "../services/email/templates.js";
import fs from "fs";
...
const appLink = (pathPart) => `${(process.env.CLIENT_ORIGIN || "http://localhost:5173").replace(/\/$/, "")}${pathPart}`;
const sendOptionalEmail = async (payload, logLabel) => {
  if (!isEmailConfigured()) return;
  try {
    await sendEmail(payload);
  } catch (err) {
    console.error(`[email:${logLabel}] failed`, err);
  }
};
...
const createPasswordResetLink = async (userId) => {
  ...
  const hours = Number(process.env.PASSWORD_RESET_HOURS || 24);
  ...
  return appLink(`/auth/reset?token=${plain}`);
};

const sendAccountCreatedEmail = async ({ userId, to, fullName }) => {
  if (!isEmailConfigured()) return;
  let resetLink;
  try {
    resetLink = await createPasswordResetLink(userId);
  } catch (err) {
    console.error("[email:account-created] reset link failed", err);
    return;
  }

  const safeName = escapeEmailHtml(fullName || "User");
  await sendOptionalEmail(
    {
      to,
      subject: "Your CBS Staff LMS account has been created",
      html: buildCbsEmailTemplate({
        title: "Your CBS Staff LMS account has been created",
        preview: "Use this secure link to set your password.",
        body: `
          ${emailParagraph(`Hello ${safeName},`)}
          ${emailParagraph("Your CBS Staff LMS account has been created. Use the secure button below to set your password.")}
          ${emailButton(resetLink, "Set your password")}
          ${emailParagraph("This link can be used once and will expire. If it expires, use Forgot Password on the login page.")}
        `,
      }),
      text: `Your CBS Staff LMS account has been created. Set your password: ${resetLink}`,
    },
    "account-created",
  );
};

const getUserAndCourseForEmail = async (userId, courseId) => {
...
const sendCourseJoinedEmail = async ({ userId, courseId, assigned = false }) => {
  if (!isEmailConfigured()) return;
  const row = await getUserAndCourseForEmail(userId, courseId);
  if (!row?.email) return;

  const courseTitle = row.course_title || "your course";
  const courseLink = appLink(`/courses/${courseId}`);
  await sendOptionalEmail(
    {
      to: row.email,
      subject: assigned ? `New course assigned: ${courseTitle}` : `You joined: ${courseTitle}`,
      html: buildCbsEmailTemplate({
        title: assigned ? "New course assigned" : "Course enrollment confirmed",
        preview: `You have been ${assigned ? "assigned to" : "joined to"} ${courseTitle}.`,
        body: `
          ${emailParagraph(`Hello ${escapeEmailHtml(row.full_name || "User")},`)}
          ${emailParagraph(`You have been ${assigned ? "assigned to" : "joined to"} <strong>${escapeEmailHtml(courseTitle)}</strong>.`)}
          ${emailButton(courseLink, "Open course")}
          ${emailParagraph("Please log in to CBS Staff LMS to view the course details and continue your learning.")}
        `,
      }),
      text: `You have been ${assigned ? "assigned to" : "joined to"} ${courseTitle}. Open course: ${courseLink}`,
    },
    "course-joined",
  );
};

const createCertificateForCourse = async (userId, courseId) => {
  ...
  const [result] = await pool.query(
    ...
    `,
    [userId, courseId, validityMonths, certificateNo, userId, courseId],
  );

  if (!result.affectedRows) return null;
  const [rows] = await pool.query(
    `
      SELECT cert.*, c.title AS course_title, u.email, p.full_name
...
const sendCertificateEmail = async (cert) => {
  ...
  const courseTitle = cert.course_title || "your course";
  const certificateLink = appLink("/my-learning");
  await sendOptionalEmail(
    {
      to: cert.email,
      subject: `Certificate completed: ${courseTitle}`,
      html: buildCbsEmailTemplate({
        title: "Certificate completed",
        preview: `Your certificate for ${courseTitle} is ready.`,
        body: `
          ${emailParagraph(`Hello ${escapeEmailHtml(cert.full_name || "User")},`)}
          ${emailParagraph(`Your certificate for <strong>${escapeEmailHtml(courseTitle)}</strong> is ready.`)}
          ${emailButton(certificateLink, "Open your certificates")}
          ${emailParagraph("You can download the certificate from My Learning in CBS Staff LMS.")}
        `,
      }),
      text: `Your certificate for ${courseTitle} is ready. Open your certificates: ${certificateLink}`,
    },
...
const upload = multer({
  storage,
  limits: { fileSize: 4000 * 1024 * 1024 }, // 200MB
...
lmsRouter.post("/enrollments", requireAuth(), async (req, res) => {
  ...
  if (existing.length) {
    ...
    if (isZeroProgress && isOldAutoStatus) {
      ...
      await sendCourseJoinedEmail({ userId, courseId });
      return res.json({ data: rows[0] });
    }
...
lmsRouter.put("/enrollments/:id/progress", requireAuth(), async (req, res) => {
  ...
  if (progress >= 100 && !hasAssessment) {
    try {
      ...
      const cert = await createCertificateForCourse(userId, courseId);
  ...
  try {
    ...
    if (passed) {
      if (assessment.module_id === null) {
        const cert = await createCertificateForCourse(userId, assessment.course_id);
...
lmsRouter.post("/training-assignments", requireAuth(["sysadmin", "instructor", "manager"]), async (req, res) => {
  try {
    ...
    await sendCourseJoinedEmail({ userId: targetUserId, courseId, assigned: true });
