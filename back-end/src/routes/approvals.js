...
import { buildCbsEmailTemplate, emailParagraph } from "../services/email/templates.js";
...
const buildEmailHtml = ({ action, planTitle, reviewerName, level, comments }) => {
  ...
  return buildCbsEmailTemplate({
    title: "Training Approval Notification",
    preview: `Your training plan has been ${safeAction}.`,
    body: `
      <h2 style="margin: 0 0 18px; color: ${headerColor}; font-size: 20px;">${headerText}</h2>
      ${emailParagraph(`<strong>Training Plan:</strong> ${safePlanTitle}`)}
      ${emailParagraph(`<strong>Reviewed by:</strong> ${safeReviewerName}`)}
      ${emailParagraph(`<strong>Level:</strong> ${safeLevel}`)}
      ${commentsBlock ? emailParagraph(`<strong>Comments:</strong> ${safeComments}`) : ""}
      ${footerBlock ? emailParagraph('<span style="color: #86efac; font-weight: 700;">Your training plan has been fully approved.</span>') : ""}
      ${emailParagraph("Log in to CBS Staff LMS to view details.")}
    `,
  });
};
